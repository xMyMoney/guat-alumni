...
public class SysDonationController extends BaseController {
    ...
    @PutMapping("/one/finish")
    public ResponseResult finishDonation(@RequestBody Donation donation) {
        BaseResp baseResp = new BaseResp();
        if (!donationService.updateById(donation)) {
            baseResp.setUpdateFailMsg();
        }
    ...
    public ResponseResult deleteDonation(@PathVariable Integer id) {
        BaseResp baseResp = new BaseResp();
        if (!donationService.delDonation(id)) {
